...
/* ── PUBLIC (but logged in): submit a new plugin for review ─────── */
router.post("/", requireUser, async (req, res) => {
  ...
  const plugin = {
    ...
    // Author comes from the logged-in account, not the form — stops
    // someone from submitting under a name that isn't theirs.
    author: String(req.user.name || req.user.email || "Unknown").trim().slice(0, 80),
